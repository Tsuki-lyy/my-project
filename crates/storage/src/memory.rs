...
impl StorageEngine for MemoryStorage {
    fn create_table(&self, name: &str, columns: Vec<(String, String)>) -> Result<()> {
        ...
        t.insert(name.to_string(), Table { columns, rows: Vec::new() });
        Ok(())
    }
    ...
    fn scan(&self, table: &str) -> Result<Vec<Vec<Value>>> {
        let t = self.tables.read().map_err(|e| Error::Storage(e.to_string()))?;
...
mod tests {
    ...
    fn setup() -> MemoryStorage {
        ...
        s.create_table("t", vec![("a".into(), "INT".into()), ("b".into(), "INT".into())])
            .unwrap();
        s
    }
